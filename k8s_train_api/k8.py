...
def configure_k8():
    ...
    credentials = service_account.Credentials.from_service_account_file('s360kubernetes.json', scopes=SCOPES)
    cluster_manager_client = ClusterManagerClient(credentials=credentials)
    cluster = cluster_manager_client.get_cluster('edustudent360', 'us-central1-a', 'autolabel-labellerr')
    ...
    def list_pods(self,job_id=None):
        ...
        for i in ret.items:
            ...
            pods.append({
                ...
                'running_time': (int(int(time.time()*1000)) - int(datetime.datetime.timestamp(i.status.start_time))),
...
# KubernetesJobs().delete_all_failed_succeeded_pods()
