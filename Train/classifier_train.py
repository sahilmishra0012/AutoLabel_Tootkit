...
def epoch_end(epoch, logs):
    ...
    credentials = service_account.Credentials.from_service_account_file(
        "edustudent360-c1d02fe3adea.json")
...
def train_model(params):
    ...
    train_batch_size = int(train_size/params.steps_per_epoch)
    val_steps = int(val_size/(train_size/params.steps_per_epoch))
    val_batch_size = int(val_size/val_steps)

    t_ds = train_data.batch(train_batch_size, drop_remainder=True)
    t_ds = t_ds.prefetch(buffer_size=int(params.steps_per_epoch/2))

    v_ds = val_data.batch(val_batch_size, drop_remainder=True)
    v_ds = v_ds.prefetch(buffer_size=int(val_batch_size/2))
